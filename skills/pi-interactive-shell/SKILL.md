...
For a foreground hands-free/dispatch overlay, `semantic: { goal: "Select the stable release channel", dynamicChoices: { enabled: true } }` opts into at most one dynamic choice per session from fresh visible output. The extension—not Jev—extracts only sequential numbered/lettered menus, explicitly keyboard-navigable menus, or one strict inline `(Y/n)`/`(y/N)` confirmation and binds opaque IDs to exact bytes. Wrapped descriptions, one visible marker, navigation/cancel help, bounded visual separator chrome, and non-executable custom-input/chat siblings are supported. Jev's dedicated dynamic choice contains only those opaque IDs plus `none`, separate from configured actions and controls. Yes/No menus and inline confirmations are code-owned `dynamic-terminal-confirmation`; other supported menus are `dynamic-terminal-choice`.

For an approved inline confirmation, the supervisor first writes exactly `y` or `n` without Enter. It sends one Enter on the next trusted visual change only if the saved viewport differs solely by the exact selected-character echo; prompt disappearance is immediate completion, and no/wrong echo, unrelated redraw, stale ownership/epoch, pause/takeover/exit, secret state, or transaction mismatch ends without Enter. This is one already-authorized transaction: no second model vote or permission prompt, no retry or rollback, and no extra dynamic choice or global-action budget. Free text, unsafe secret/lifecycle prompts, multiple or ambiguous prompts, and unsupported formats produce no dynamic input. Global `jev.semanticPermissions` rules use `allow`, `ask`, or `deny`; deny wins, unmatched asks, and project/tool config cannot select or weaken operation identity. Prefer `ask` for both dynamic operation kinds unless a narrower trusted rule is intended. Ask uses Pi's one-time confirmation dialog bound to the exact session/operation/generation/hash; Pi owns delivery of that dialog. This feature is not general unattended CLI operation or a command sandbox. Configured fixed actions are unchanged and retain all existing gates.
