...
`quietIntervalMs` performs exactly one observe-only reassessment per inactivity episode (default 2,000ms, bounded 250–60,000 and still gated by `minIntervalMs`), including no-output sessions. Quiet evaluation never writes bytes.

For a foreground hands-free/dispatch overlay, `semantic: { goal: "Select the stable release channel", dynamicChoices: { enabled: true } }` opts into at most one dynamic interaction per session from fresh visible output. Before ordinary menus, the extension extracts only a bounded 2–8 item multi-select with one cursor and an exact counted footer (`<N> choices total • ↑↓ navigate • space select • ⏎ submit`) whose total equals the contiguous visible rows. Default Inquirer checkbox output is unsupported because silent pagination cannot prove full-list visibility; missing/wrong counts and other malformed evidence fail closed instead of becoming a single-choice menu. Multi-select question metadata contains ordered opaque IDs/labels and typed desired-state questions, never executable bytes. Empty apply differs from abstain; thresholds are 0.90 to select/apply and 0.10 to unselect. Provider caches must key the exact ordered descriptors and invalidate entries when the question schema changes. Multi-select uses `dynamic-terminal-multi-select`; existing confirmation and choice identities remain separate.

An approved multi-select re-grounds the exact trusted viewport, creates an immutable desired-set plan, and performs one ArrowUp/ArrowDown/Space per exactly verified redraw, followed by one Enter only after the desired set is rendered; completion requires prompt disappearance. The transaction uses one authorization and global reservation, has a `3*N+1` write bound, and permits no second model vote, retry, rollback, or interleaved control. Mismatch, write failure, stale ownership/epoch, pause/takeover/exit, secret state, or stop clears it without replay or submit and reports incomplete state. Pre-action failures write zero bytes. Inline confirmation behavior is unchanged. Global permission deny wins, unmatched asks through Pi's one-time UI, and allow bypasses the dialog; project/tool config cannot weaken any operation identity. Configured fixed actions remain unchanged.

Semantic event handoffs include a bounded redacted excerpt and content-sensitive identity. To answer an ordinary input handoff, one `semanticReply` must repeat the exact event session, decision, generation, and identity plus one bounded single-line response. Runtime freshness, ownership, secret/restriction, reload/takeover/exit, and trusted global `semantic-reply` permission checks happen immediately before the write; stale, denied, rejected, and UI-unavailable requests write nothing. This is not arbitrary command authority, and ordinary manual input is unchanged.
