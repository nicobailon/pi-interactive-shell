...
export interface OutputBlock { readonly range: SourceRange; readonly text: string; readonly kind?: "output" | "diagnostic" | "result" | "progress" | "prompt" }
export interface OutputSelectionInput {
	readonly source: { readonly identity: SourceIdentity; readonly text: string; readonly complete: boolean; readonly kind: OutputKind };
...
const SECRET = /(?:api[_-]?key|password|secret|token|authorization)\s*[:=]/i;
...
const bypassReason = (input: OutputSelectionInput): string | undefined => {
	...
	if (input.source.text.length <= OUTPUT_SELECTOR_POLICY.visibleChars || input.blocks.length <= 2) return "short-output";
	if (SECRET.test(input.source.text)) return "secret-like-input";
	return undefined;
};
...
function requestFor(input: OutputSelectionInput, positions: readonly number[], redact: TerminalRedactor): { request: JevEvaluationRequest; accounting: { payload: number; single: number; all: number } } {
	const blocks = positions.map((position) => ({ position, range: { start: input.blocks[position]!.range.start, end: input.blocks[position]!.range.end }, text: redact(input.blocks[position]!.text) }));
...
function render(input: OutputSelectionInput, retained: SourceRange[], omissions: SourceRange[]): { text: string; excerpts: { range: SourceRange; text: string }[] } {
	const excerpts = mergeRanges(retained).map((range) => ({ range, text: input.source.text.slice(range.start, range.end) })); let text = "";
...
export async function selectOutput(input: OutputSelectionInput, options: OutputSelectorOptions): Promise<OutputSelectionResult> {
	...
	if (reason) { const excerpts = fullRange(input.source.text).map((range) => ({ range, text: input.source.text })); if (input.source.text.length > OUTPUT_SELECTOR_POLICY.visibleChars) return { status: "pagination-required", reason, source: identity, rawSourceReference, excerpts: [], text: "", recoveryRanges: fullRange(input.source.text), audit: auditBase([], [], [], fullRange(input.source.text), reason !== "incomplete-source") }; if (reason === "incomplete-source") return { status: "unavailable", reason, source: identity, rawSourceReference, excerpts, text: input.source.text, audit: auditBase([], fullRange(input.source.text), [], [], false) }; return { status: "unchanged", reason, source: identity, rawSourceReference, excerpts, text: input.source.text, audit: emptyAudit }; }
	...
	const clock = options.clock ?? { now: Date.now, setTimeout: (cb: () => void, ms: number) => setTimeout(cb, ms), clearTimeout: (h: unknown) => clearTimeout(h as NodeJS.Timeout) }; const outerStart = clock.now(); let unavailable = false;
