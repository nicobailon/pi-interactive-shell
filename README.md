...
The extension conservatively extracts a fresh sequential numbered/lettered menu, an explicitly keyboard-navigable menu, or one strict inline `(Y/n)`/`(y/N)` confirmation from the visible viewport. Wrapped descriptions and a single visible selection marker are supported; recognized navigation/cancel help, bounded visual separator chrome, and unsupported custom-input/chat rows may remain visible without becoming executable choices. This foreground-only feature can execute one dynamic choice per session. Jev receives a dedicated choice containing only code-owned opaque visible-option IDs plus `none`; it never receives or supplies terminal input, and dynamic options do not compete with configured actions or action controls. Code resolves the chosen ID to exact extracted bytes. Yes/No menus and inline confirmations are code-classified as `dynamic-terminal-confirmation`; other supported menus use `dynamic-terminal-choice`.

An approved inline confirmation is one bounded transaction, not general multi-step autonomy. After all normal confidence, permission, freshness, ownership, secret, and budget checks, the supervisor writes exactly `y` or `n` without Enter. On the next trusted visual change it sends one Enter only when the saved viewport changed solely by echoing that exact character after the exact prompt; prompt disappearance means the CLI already consumed the key, while an unchanged/wrong echo, redraw, takeover, reload, pause, exit, secret prompt, or identity mismatch ends the transaction without Enter. There is no second model vote, permission prompt, retry, rollback, or second dynamic-budget charge. Free text, unsafe secret/lifecycle prompts, ambiguous layouts, multiple confirmations, and other unsupported interactions produce no dynamic input. Global `jev.semanticPermissions` rules are the trusted policy source for each classification; project/tool configuration cannot add or weaken them. Prefer `ask` for both operation kinds unless a narrower trusted rule is intended. `deny` always blocks; `ask` opens Pi's confirmation dialog and grants one request bound to the session, operation, generation, and observation hash; `allow` skips the dialog. Elapsed/quiet time alone does not invalidate that hash, but terminal, session, or operation changes do. `stop_automation` prevents later and pending dynamic choices for the session. This is not general unattended CLI operation or a command sandbox. Existing configured fixed actions are unchanged and retain their ownership, freshness, secret, cooldown, dedupe, and budget gates.
