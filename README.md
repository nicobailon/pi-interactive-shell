...
interactive_shell({
  ...
  monitor: { semantic: {
    ...
    dynamicChoices: { enabled: true, maxActions: 1 }
  } }
})
```

The extension conservatively extracts a fresh numbered, lettered, or explicitly keyboard-navigable menu from the visible viewport. Jev chooses only among code-owned opaque IDs plus `observe_again`, `notify_pi`, and `stop_automation`; it never supplies terminal input. Code resolves the chosen ID to the exact extracted bytes. Global `jev.semanticPermissions` is the trusted policy source: `dynamic-terminal-choice` controls these choices and exact `launch-command` rules govern configured fixed actions. Project/tool configuration cannot add or weaken policy. `deny` always blocks; `ask` opens Pi's real confirmation UI and grants one request bound to the session, operation, generation, and observation hash; `allow` skips the modal. Missing UI, background transfer, rejection, reload, takeover, exit, expiry, stale output, provider failure, ambiguity, secrets, lifecycle-like content, or unsupported menus result in no dynamic input. Existing fixed actions retain their ownership, freshness, secret, cooldown, dedupe, and budget gates after permission approval.
